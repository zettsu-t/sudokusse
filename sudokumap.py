...
class SudokuConstAll:
    ...
    def get_cells_str(self, cell_str_func, length):
        str = '{'
        str += ','.join(list(map(cell_str_func, range(0, length))))
        str += '}'
        return str
    ...
    def get_rows_str_set(self):
        return list(map(self.get_row_str, range(0, SUDOKU_ROWS_IN_MAP)))
    ...
    def get_columns_str_set(self):
        return list(map(self.get_column_str,
                        range(0, SUDOKU_COLUMNS_IN_MAP)))

    def get_box_line_str(self, x, y, ofs_x):
        f = lambda ofs_y: str(
            x + ofs_x + (y + ofs_y) * SUDOKU_COLUMNS_IN_MAP)
        return ','.join(list(map(f, range(0, SUDOKU_BOX_VERTICAL_SIZE))))
    ...
    def get_reverse_group_column_str(self, column):
        ...
        return ','.join(list(map(f, range(0, SUDOKU_ROWS_IN_MAP))))
    ...
    def get_reverse_group_str(self):
        ...
        str += ",\n".join(list(map(f, range(0, SUDOKU_COLUMNS_IN_MAP))))
        str += "\n};\n"
        return str
    ...
    def count_bits(self, bitmap):
        count = 0
        bitmask = 1
        for i in range(0,SUDOKU_CELLS_IN_GROUP):
    ...
    def get_cell_lookup_str(self):
        ...
        str = ','.join(list(map(self.get_element_str,
                                range(0, 2 ** SUDOKU_CELLS_IN_GROUP))))
